...
class Player extends entity_1.Entity {
	...
	onTick() {
		...
		if (!this.vehicle.isFlight()) {
			if (this.hasState(state_type_1.StateType.IN_RIVER) && !this.hasState(state_type_1.StateType.IN_BRIDGE) && !this.direction) this.direction = 12;
	...
	failQuests(...types) {
		for (const type of types) {
			if (this.quests[type] !== quest_type_1.QuestState.PROCCESS) continue;
	...
	ruinQuests() {
		for (let i = quest_type_1.QuestType.DRAGON_ORB; i < quest_type_1.QuestType.GREEN_GEM + 1; i++) {
			if (this.quests[i] !== quest_type_1.QuestState.PROCCESS) continue;
			this.failQuests(i);
		}
	...
	updateSpeed() {
		...
		if (this.vehicle?.id) {
			this.vehicleSpeed =
				(this.vehicle.id === item_type_1.ItemType.BABY_LAVA
					? config.speed_mount_baby_lava
					: this.vehicle.id === item_type_1.ItemType.BABY_DRAGON
					? config.speed_mount_baby_dragon
					: this.vehicle.id === item_type_1.ItemType.BABY_MAMMOTH
					? config.speed_mount_baby_mammoth
					: this.vehicle.id === item_type_1.ItemType.PLANE
					? config.speed_mount_plane
					: config.speed_mount_plane) / 1000;
			if (this.hasState(state_type_1.StateType.IN_DESERT)) {
				this.vehicleSpeed =
					(this.vehicle.id === item_type_1.ItemType.BABY_LAVA ? config.speed_mount_baby_lava_desert : this.vehicle.id === item_type_1.ItemType.BABY_DRAGON ? config.speed_mount_baby_dragon_desert : config.speed_mount_plane_desert) / 1000;
			}
			if (this.hasState(state_type_1.StateType.IN_WINTER)) {
				this.vehicleSpeed =
					(this.vehicle.id === item_type_1.ItemType.BABY_LAVA ? config.speed_mount_baby_lava_winter : this.vehicle.id === item_type_1.ItemType.BABY_DRAGON ? config.speed_mount_baby_dragon_winter : config.speed_mount_plane_winter) / 1000;
			}
			if (this.hasState(state_type_1.StateType.IN_WINTER)) {
				this.vehicleSpeed =
					(this.vehicle.id === item_type_1.ItemType.BABY_LAVA ? config.speed_mount_baby_lava_winter : this.vehicle.id === item_type_1.ItemType.BABY_DRAGON ? config.speed_mount_baby_dragon_winter : config.speed_mount_plane_winter) / 1000;
			...
			if (this.hasState(state_type_1.StateType.IN_WATER) && !this.hasState(state_type_1.StateType.IN_BRIDGE)) {
				this.vehicleSpeed =
					(this.vehicle.id === item_type_1.ItemType.BABY_LAVA ? config.speed_mount_baby_lava_water : this.vehicle.id === item_type_1.ItemType.BABY_DRAGON ? config.speed_mount_baby_dragon_water : config.speed_mount_plane_water) / 1000;
			}
			if (this.hasState(state_type_1.StateType.IN_LAVA_BIOME)) {
				this.vehicleSpeed = (this.vehicle.id === item_type_1.ItemType.BABY_LAVA ? config.speed_mount_baby_lava_lava : this.vehicle.id === item_type_1.ItemType.BABY_DRAGON ? config.speed_mount_baby_dragon_lava : config.speed_mount_plane_lava) / 1000;
			}
		} else {
		...
		if (this.vehicle.id) {
			const flyTime = this.vehicle.id === item_type_1.ItemType.BABY_LAVA || this.vehicle.id === item_type_1.ItemType.NIMBUS ? 3 : this.vehicle.id === item_type_1.ItemType.BABY_DRAGON ? 5 : this.vehicle.id === item_type_1.ItemType.PLANE ? 15 : 10;
			...
			if (this.vehicle.id === item_type_1.ItemType.NIMBUS && this.helmet.id !== item_type_1.ItemType.WITCH_HAT && !((this.hasState(state_type_1.StateType.IN_ROOF) || obstacles.length) && this.flight)) this.speed = 0.03;
		}
		if (obstacles.length) {
	...
	serialize() {
		...
		return [
			...
			this.vehicle.id,
			this.createdAt - now,
			this.lastEvent.map((x) => (x === -1 ? -1 : x - now)),
			this.account ? this.account.name : undefined,
			this.account ? this.account.password : undefined,
			this.cosmetics.skin,
			this.cosmetics.accessory,
			this.cosmetics.book,
			this.cosmetics.bag,
			this.cosmetics.crate,
			this.cosmetics.dead,
		];
	}
	delete() {
	...
	onDead(damager) {
		...
		if (damager instanceof Player) {
			...
			health_system_1.default.heal(damager, 50);
		}
	}
}
