...
class CombatSystem {
	...
	sameGearBuff(player) {
		switch (player.helmet.id) {
			case item_type_1.ItemType.WOOD_HELMET:
				if (player.right.id == item_type_1.ItemType.WOOD_SWORD || player.right.id == item_type_1.ItemType.WOOD_SPEAR) return true;
				break;

			case item_type_1.ItemType.STONE_HELMET:
				if (player.right.id == item_type_1.ItemType.STONE_SWORD || player.right.id == item_type_1.ItemType.STONE_SPEAR) return true;
				break;

			case item_type_1.ItemType.GOLD_HELMET:
				if (player.right.id == item_type_1.ItemType.GOLD_SWORD || player.right.id == item_type_1.ItemType.GOLD_SPEAR) return true;
				break;

			case item_type_1.ItemType.DIAMOND_HELMET:
				if (player.right.id == item_type_1.ItemType.DIAMOND_SWORD || player.right.id == item_type_1.ItemType.DIAMOND_SPEAR) return true;
				break;

			case item_type_1.ItemType.AMETHYST_HELMET:
				if (player.right.id == item_type_1.ItemType.AMETHYST_SWORD || player.right.id == item_type_1.ItemType.AMETHYST_SPEAR) return true;
				break;

			case item_type_1.ItemType.REIDITE_HELMET:
				if (player.right.id == item_type_1.ItemType.REIDITE_SWORD || player.right.id == item_type_1.ItemType.REIDITE_SPEAR) return true;
				break;

			case item_type_1.ItemType.DRAGON_HELMET:
				if (player.right.id == item_type_1.ItemType.DRAGON_SWORD || player.right.id == item_type_1.ItemType.DRAGON_SPEAR) return true;
				break;

			case item_type_1.ItemType.LAVA_HELMET:
				if (player.right.id == item_type_1.ItemType.LAVA_SWORD || player.right.id == item_type_1.ItemType.LAVA_SPEAR) return true;
				break;
	...
	tick(player) {
		const now = Date.now();
		if (this.server.config.disable_attack === 1 || !player.hasState(state_type_1.StateType.ATTACK) || now - player.timestamps.get("attack") < 500) return;
		player.addAction(action_type_1.ActionType.ATTACK);
		player.timestamps.set("attack", now);
		let buff = 0;
		if (this.sameGearBuff(player)) buff = 3;
		...
		if (canDig) {
			const item =
				...
				player.hasState(state_type_1.StateType.IN_ISLAND) || player.hasState(state_type_1.StateType.IN_DESERT)
					? item_type_1.ItemType.SAND
					: player.hasState(state_type_1.StateType.IN_LAVA_BIOME) || player.hasState(state_type_1.StateType.IN_FOREST)
					? item_type_1.ItemType.GROUND
					: player.hasState(state_type_1.StateType.IN_WINTER)
					? item_type_1.ItemType.ICE
					: 0;
		...
		for (const unit of damaged) {
			...
			if (unit instanceof player_1.default) {
				...
				if (
					player.right.id === item_type_1.ItemType.HAND &&
					isHood &&
					!peasant &&
					!player.hasState(state_type_1.StateType.FIRE) &&
					!unit.hasState(state_type_1.StateType.FIRE) &&
				...
				health_system_1.default.damage(unit, (player.right.damage + unit.defense + buff) * (team_system_1.default.isAlly(player, unit) ? 0.3 : 1), action_type_1.ActionType.HURT, player);
		...
		if (empty && player.client) player.client.sendBinary(cached_packets_1.CachedPackets.get(packets_1.ClientPackets.EMPTY_RES));
		if (dontHarvest && player.client) player.client.sendBinary(cached_packets_1.CachedPackets.get(packets_1.ClientPackets.DONT_HARVEST));
